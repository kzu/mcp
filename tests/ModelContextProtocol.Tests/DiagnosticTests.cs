...
public class DiagnosticTests
{
    ...
    public async Task Session_TracksActivities()
    {
            ...
            .Build())
        {
            await RunConnected(async (client, server) =>
            {
                ...
                await tool.InvokeAsync(new() { ["amount"] = 42 }, TestExecutionContext.Current.CancellationToken);
            }, clientToServerLog);
    ...
    public async Task Session_FailedToolCall()
    {
            ...
            .Build())
        {
            await RunConnected(async (client, server) =>
            {
                await client.CallToolAsync("Throw", cancellationToken: TestExecutionContext.Current.CancellationToken);
                await Assert.ThrowsAsync<McpProtocolException>(async () => await client.CallToolAsync("does-not-exist", cancellationToken: TestExecutionContext.Current.CancellationToken));
            }, []);
    ...
    public async Task Session_McpAttributesAddedToOuterExecuteToolActivity()
    {
            ...
            .Build())
        {
            await RunConnected(async (client, server) =>
            {
                ...
                var tool = (await client.ListToolsAsync(cancellationToken: TestExecutionContext.Current.CancellationToken))
    ...
    private static async Task RunConnected(Func<McpClient, McpServer, Task> action, List<string> clientToServerLog)
    {
        ...
        {
            serverTask = server.RunAsync(TestExecutionContext.Current.CancellationToken);
