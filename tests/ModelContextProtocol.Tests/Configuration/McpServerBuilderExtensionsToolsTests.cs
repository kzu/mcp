...
public partial class McpServerBuilderExtensionsToolsTests : ClientServerTestBase
{
    ...
    public async Task Can_Create_Multiple_Servers_From_Options_And_List_Registered_Tools()
    {
        ...
        for (int i = 0; i < 2; i++)
        {
            ...
            var serverRunTask = server.RunAsync(TestExecutionContext.Current.CancellationToken);
    ...
    public async Task Can_Be_Notified_Of_Tool_Changes()
    {
            ...
            }))
        {
            ...
            tools = await client.ListToolsAsync(cancellationToken: TestExecutionContext.Current.CancellationToken);
    ...
    public async Task WithTools_TargetInstance_UsesTarget()
    {
        ...
        var result = await tool.InvokeAsync(new RequestContext<CallToolRequestParams>(new Mock<McpServer>().Object, new JsonRpcRequest { Method = "test", Id = new RequestId("1") }, new() { Name = "" }), TestExecutionContext.Current.CancellationToken);
