...
public partial class McpServerScopedTests : ClientServerTestBase
{
    ...
    public async Task InjectScopedServiceAsArgument()
    {
        ...
        var tools = await client.ListToolsAsync(new RequestOptions { JsonSerializerOptions = McpServerScopedTestsJsonContext.Default.Options }, TestExecutionContext.Current.CancellationToken);
        ...
        for (int i = 1; i <= 10; i++)
        {
            Assert.Contains("\"Scoped\"", JsonSerializer.Serialize(await tool.InvokeAsync(cancellationToken: TestExecutionContext.Current.CancellationToken), McpJsonUtilities.DefaultOptions));
