...
/// </summary>
public sealed class McpServerResourceRoutingTests() : ClientServerTestBase(startServer: false)
{
    /// <summary>
        ...
        string expectedResult)
    {
        ...
        var result = await client.ReadResourceAsync(uri, null, TestExecutionContext.Current.CancellationToken);
        ...
        string uri)
    {
        ...
        var ex = await Assert.ThrowsAsync<McpProtocolException>(async () =>
            await client.ReadResourceAsync(uri, null, TestExecutionContext.Current.CancellationToken));
    ...
    public async Task MultipleTemplatedResources_MatchesCorrectResource()
    {
        ...
        var nonTemplatedResult = await client.ReadResourceAsync("test://resource/non-templated", null, TestExecutionContext.Current.CancellationToken);
        Assert.Equal("static", ((TextResourceContents)nonTemplatedResult.Contents[0]).Text);

        // Templated URI
        var templatedResult = await client.ReadResourceAsync("test://resource/12345", null, TestExecutionContext.Current.CancellationToken);
        Assert.Equal("template: 12345", ((TextResourceContents)templatedResult.Contents[0]).Text);

        // Exact match for templated URI
        var exactTemplatedResult = await client.ReadResourceAsync("test://resource/{id}", null, TestExecutionContext.Current.CancellationToken);
        Assert.Equal("template: {id}", ((TextResourceContents)exactTemplatedResult.Contents[0]).Text);

        // Templated URI with query params
        var paramsResult = await client.ReadResourceAsync("test://params?a1=a&a2=b&a3=c", null, TestExecutionContext.Current.CancellationToken);
        ...
        var pathResult = await client.ReadResourceAsync("file://foo/examples/example.cs", null, TestExecutionContext.Current.CancellationToken);
        ...
        var mcpEx = await Assert.ThrowsAsync<McpProtocolException>(async () => await client.ReadResourceAsync("test://params{?a1,a2,a3}", null, TestExecutionContext.Current.CancellationToken));
