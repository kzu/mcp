...
{
    public McpServerBuilderExtensionsResourcesTests()
        : base()
    {
    }
    ...
    public async Task Can_List_And_Call_Registered_Resources()
    {
        ...
        var result = await resource.ReadAsync(cancellationToken: TestExecutionContext.Current.CancellationToken);
    ...
    public async Task Can_List_And_Call_Registered_ResourceTemplates()
    {
        ...
        var resources = await client.ListResourceTemplatesAsync(cancellationToken: TestExecutionContext.Current.CancellationToken);
        ...
        var result = await resource.ReadAsync(new Dictionary<string, object?>() { ["name"] = "hello" }, cancellationToken: TestExecutionContext.Current.CancellationToken);
        Assert.NotNull(result);
        Assert.Single(result.Contents);
    ...
    public async Task Can_Be_Notified_Of_Resource_Changes()
    {
        ...
        var notificationRead = listChanged.Reader.ReadAsync(TestExecutionContext.Current.CancellationToken);
            ...
            }))
        {
            ...
            resources = await client.ListResourcesAsync(cancellationToken: TestExecutionContext.Current.CancellationToken);
            Assert.Equal(8, resources.Count);
            Assert.Contains(resources, t => t.Name == "NewResource");

            notificationRead = listChanged.Reader.ReadAsync(TestExecutionContext.Current.CancellationToken);
            Assert.False(notificationRead.IsCompleted);
    ...
    public async Task AttributeProperties_Propagated()
    {
        ...
        var resourceTemplates = await client.ListResourceTemplatesAsync(cancellationToken: TestExecutionContext.Current.CancellationToken);
