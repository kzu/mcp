...
public class McpServerBuilderExtensionsMessageFilterTests() : ClientServerTestBase(startServer: false)
{
    ...
    public async Task AddOutgoingMessageFilter_Sees_Responses_Notifications_And_Requests()
    {
        ...
        await client.CallToolAsync("progress-tool", progress: progress, cancellationToken: TestExecutionContext.Current.CancellationToken);
    ...
    public async Task AddOutgoingMessageFilter_Can_Send_Additional_Messages()
    {
        ...
        var extraMessage = await extraNotificationReceived.Task.WaitAsync(TestConstants.DefaultTimeout, TestExecutionContext.Current.CancellationToken);
    ...
    public async Task AddIncomingMessageFilter_SkipNext_DoesNotLogSendingResponse()
    {
        ...
        McpServerBuilder
            ...
            {
                // Skip processing tools/list requests â€” handler never runs, no response sent
                if (context.JsonRpcMessage is JsonRpcRequest request && request.Method == RequestMethods.ToolsList)
                {
                    return Task.CompletedTask;
                }
    ...
    public async Task AddIncomingMessageFilter_CallsNext_LogsSendingResponse()
    {
        ...
        McpServerBuilder
            .WithMessageFilters(filters => filters.AddIncomingFilter((next) => (context, cancellationToken) =>
            {
                // Pass through â€” handler runs, response is sent
                return next(context, cancellationToken);
            }))
            .WithTools<TestTool>();
    ...
    public async Task AddIncomingMessageFilter_Items_Flow_To_CallTool_Handler()
    {
        ...
        await client.CallToolAsync("simple-tool", cancellationToken: TestExecutionContext.Current.CancellationToken);
