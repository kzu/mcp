...
public class StdioClientTransportTests() : LoggedTest()
{
    ...
    public async Task ConnectAsync_DoesNotLogEnvironmentVariablesAtTrace()
    {
        ...
        await using var _ = await transport.ConnectAsync(TestExecutionContext.Current.CancellationToken);
    ...
    public async Task CreateAsync_ValidProcessInvalidServer_Throws()
    {
        ...
        await Assert.ThrowsAnyAsync<IOException>(() => McpClient.CreateAsync(transport, loggerFactory: LoggerFactory, cancellationToken: TestExecutionContext.Current.CancellationToken));
    }

    [Test]
    public async Task CreateAsync_ValidProcessInvalidServer_StdErrCallbackInvoked()
    {
        Assert.SkipUnless(IsStdErrCallbackSupported, "Platform not supported by this test.");
    ...
    public async Task EscapesCliArgumentsCorrectly(string? cliArgumentValue)
    {
        ...
        var result = await client.CallToolAsync("echoCliArg", cancellationToken: TestExecutionContext.Current.CancellationToken);
    ...
    public async Task SendMessageAsync_Should_Use_LF_Not_CRLF()
    {
        ...
        await using var sessionTransport = await transport.ConnectAsync(TestExecutionContext.Current.CancellationToken);

        var message = new JsonRpcRequest { Method = "test", Id = new RequestId(44) };

        await sessionTransport.SendMessageAsync(message, TestExecutionContext.Current.CancellationToken);
    ...
    public async Task ReadMessagesAsync_Should_Accept_CRLF_Delimited_Messages()
    {
        ...
        var canRead = await sessionTransport.MessageReader.WaitToReadAsync(TestExecutionContext.Current.CancellationToken);
