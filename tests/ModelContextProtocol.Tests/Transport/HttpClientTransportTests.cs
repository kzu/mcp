...
public class HttpClientTransportTests : LoggedTest
{
    ...
    public async Task ConnectAsync_Throws_Exception_On_Failure()
    {
        ...
        var exception = await Assert.ThrowsAsync<Exception>(() => transport.ConnectAsync(TestContext.CurrentContext.CancellationToken));
    ...
    public async Task ConnectAsync_Throws_HttpRequestException_With_ResponseBody_On_ErrorStatusCode()
    {
        ...
        var httpException = await Assert.ThrowsAsync<HttpRequestException>(() => transport.ConnectAsync(TestContext.CurrentContext.CancellationToken));
    ...
    public async Task StreamableHttp_InitialGetSseConnection_DoesNotCountAgainstMaxReconnectionAttempts()
    {
        ...
        await allGetRequestsDone.Task.WaitAsync(TimeSpan.FromSeconds(10), TestContext.CurrentContext.CancellationToken);
