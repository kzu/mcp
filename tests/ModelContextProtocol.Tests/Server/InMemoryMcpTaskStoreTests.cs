...
public class InMemoryMcpTaskStoreTests : LoggedTest
{
    ...
    public async Task CreateTaskAsync_CreatesTaskWithUniqueId()
    {
        ...
        // Act
        var task = await store.CreateTaskAsync(metadata, requestId, request, "session-1", TestExecutionContext.Current.CancellationToken);
    ...
    public async Task CreateTaskAsync_GeneratesUniqueTaskIds()
    {
        ...
        // Act
        var task1 = await store.CreateTaskAsync(metadata, new RequestId("req-1"), new JsonRpcRequest { Method = "test" }, null, TestExecutionContext.Current.CancellationToken);
        var task2 = await store.CreateTaskAsync(metadata, new RequestId("req-2"), new JsonRpcRequest { Method = "test" }, null, TestExecutionContext.Current.CancellationToken);
    ...
    public async Task CreateTaskAsync_AppliesTtlFromMetadata()
    {
        ...
        // Act
        var task = await store.CreateTaskAsync(metadata, new RequestId("req-1"), new JsonRpcRequest { Method = "test" }, null, TestExecutionContext.Current.CancellationToken);
    ...
    public async Task GetTaskAsync_ReturnsTaskById()
    {
        ...
        using var store = new InMemoryMcpTaskStore();
        var metadata = new McpTaskMetadata();
        var created = await store.CreateTaskAsync(metadata, new RequestId("req-1"), new JsonRpcRequest { Method = "test" }, null, TestExecutionContext.Current.CancellationToken);

        // Act
        var retrieved = await store.GetTaskAsync(created.TaskId, null, TestExecutionContext.Current.CancellationToken);
    ...
    public async Task GetTaskAsync_ReturnsNullForNonexistentTask()
    {
        ...
        // Act
        var task = await store.GetTaskAsync("nonexistent-id", null, TestExecutionContext.Current.CancellationToken);
    ...
    public async Task GetTaskAsync_EnforcesSessionIsolation()
    {
        ...
        using var store = new InMemoryMcpTaskStore();
        var metadata = new McpTaskMetadata();
        var task = await store.CreateTaskAsync(metadata, new RequestId("req-1"), new JsonRpcRequest { Method = "test" }, "session-1", TestExecutionContext.Current.CancellationToken);

        // Act
        var sameSession = await store.GetTaskAsync(task.TaskId, "session-1", TestExecutionContext.Current.CancellationToken);
        var differentSession = await store.GetTaskAsync(task.TaskId, "session-2", TestExecutionContext.Current.CancellationToken);
    ...
    public async Task StoreTaskResultAsync_StoresResultForCompletedTask()
    {
        ...
        // Act
        await store.StoreTaskResultAsync(task.TaskId, McpTaskStatus.Completed, resultElement, null, TestExecutionContext.Current.CancellationToken);

        // Assert
        var retrieved = await store.GetTaskAsync(task.TaskId, null, TestExecutionContext.Current.CancellationToken);
    ...
    public async Task StoreTaskResultAsync_EnforcesSessionIsolation()
    {
        ...
        await Assert.ThrowsAsync<InvalidOperationException>(
            () => store.StoreTaskResultAsync(task.TaskId, McpTaskStatus.Completed, resultElement, "session-2", TestExecutionContext.Current.CancellationToken));
    }
    ...
    public async Task StoreTaskResultAsync_ThrowsForNonTerminalStatus()
    {
        ...
        await Assert.ThrowsAsync<ArgumentException>(
            () => store.StoreTaskResultAsync(task.TaskId, McpTaskStatus.Working, resultElement, null, TestExecutionContext.Current.CancellationToken));
    }
    ...
    public async Task GetTaskResultAsync_ReturnsStoredResult()
    {
        ...
        // Act
        var retrieved = await store.GetTaskResultAsync(task.TaskId, null, TestExecutionContext.Current.CancellationToken);
    ...
    public async Task GetTaskResultAsync_EnforcesSessionIsolation()
    {
        ...
        var result = new CallToolResult { Content = [new TextContentBlock { Text = "Success" }] };
        var resultElement = JsonSerializer.SerializeToElement(result, McpJsonUtilities.DefaultOptions);
        await store.StoreTaskResultAsync(task.TaskId, McpTaskStatus.Completed, resultElement, "session-1", TestExecutionContext.Current.CancellationToken);

        // Act & Assert
        await Assert.ThrowsAsync<InvalidOperationException>(
            () => store.GetTaskResultAsync(task.TaskId, "session-2", TestExecutionContext.Current.CancellationToken));
    }
    ...
    public async Task UpdateTaskStatusAsync_UpdatesStatus()
    {
        ...
        // Act
        await store.UpdateTaskStatusAsync(task.TaskId, McpTaskStatus.Working, "Processing...", null, TestExecutionContext.Current.CancellationToken);

        // Assert
        var updated = await store.GetTaskAsync(task.TaskId, null, TestExecutionContext.Current.CancellationToken);
    ...
    public async Task UpdateTaskStatusAsync_UpdatesLastUpdatedAt()
    {
        ...
        // Act
        await store.UpdateTaskStatusAsync(task.TaskId, McpTaskStatus.Working, null, null, TestExecutionContext.Current.CancellationToken);
    ...
    public async Task ListTasksAsync_ReturnsAllTasks()
    {
        // Arrange
        using var store = new InMemoryMcpTaskStore();
        var task1 = await store.CreateTaskAsync(new McpTaskMetadata(), new RequestId("req-1"), new JsonRpcRequest { Method = "test" }, null, TestExecutionContext.Current.CancellationToken);
        var task2 = await store.CreateTaskAsync(new McpTaskMetadata(), new RequestId("req-2"), new JsonRpcRequest { Method = "test" }, null, TestExecutionContext.Current.CancellationToken);

        // Act
        var result = await store.ListTasksAsync(cancellationToken: TestExecutionContext.Current.CancellationToken);
    ...
    public async Task ListTasksAsync_FiltersBySession()
    {
        // Arrange
        using var store = new InMemoryMcpTaskStore();
        var task1 = await store.CreateTaskAsync(new McpTaskMetadata(), new RequestId("req-1"), new JsonRpcRequest { Method = "test" }, "session-1", TestExecutionContext.Current.CancellationToken);
        var task2 = await store.CreateTaskAsync(new McpTaskMetadata(), new RequestId("req-2"), new JsonRpcRequest { Method = "test" }, "session-2", TestExecutionContext.Current.CancellationToken);

        // Act
        var session1Result = await store.ListTasksAsync(sessionId: "session-1", cancellationToken: TestExecutionContext.Current.CancellationToken);
        var session2Result = await store.ListTasksAsync(sessionId: "session-2", cancellationToken: TestExecutionContext.Current.CancellationToken);
    ...
    public async Task ListTasksAsync_SupportsPagination()
    {
        ...
        for (int i = 0; i < 150; i++)
        {
            await store.CreateTaskAsync(new McpTaskMetadata(), new RequestId($"req-{i}"), new JsonRpcRequest { Method = "test" }, null, TestExecutionContext.Current.CancellationToken);
        }

        // Act - First page
        var firstPageResult = await store.ListTasksAsync(cancellationToken: TestExecutionContext.Current.CancellationToken);
        
        // Act - Second page
        var secondPageResult = await store.ListTasksAsync(cursor: firstPageResult.NextCursor, cancellationToken: TestExecutionContext.Current.CancellationToken);
    ...
    public async Task CancelTaskAsync_CancelsTask()
    {
        ...
        // Act
        var cancelled = await store.CancelTaskAsync(task.TaskId, null, TestExecutionContext.Current.CancellationToken);
    ...
    public async Task CancelTaskAsync_IsIdempotent()
    {
        ...
        // First cancellation
        await store.CancelTaskAsync(task.TaskId, null, TestExecutionContext.Current.CancellationToken);

        // Act - Second cancellation
        var result = await store.CancelTaskAsync(task.TaskId, null, TestExecutionContext.Current.CancellationToken);
    ...
    public async Task CancelTaskAsync_DoesNotCancelCompletedTask()
    {
        ...
        // Act
        var cancelResult = await store.CancelTaskAsync(task.TaskId, null, TestExecutionContext.Current.CancellationToken);
    ...
    public async Task CancelTaskAsync_EnforcesSessionIsolation()
    {
        ...
        // Act & Assert
        await Assert.ThrowsAsync<InvalidOperationException>(
            () => store.CancelTaskAsync(task.TaskId, "session-2", TestExecutionContext.Current.CancellationToken));
    }
    ...
    public async Task Dispose_StopsCleanupTimer()
    {
        ...
        await store.CreateTaskAsync(metadata, new RequestId("req-1"), new JsonRpcRequest { Method = "test" }, null, TestExecutionContext.Current.CancellationToken);
    ...
    public async Task CleanupExpiredTasks_RemovesExpiredTasks()
    {
        ...
        var resultBefore = await store.ListTasksAsync(cancellationToken: TestExecutionContext.Current.CancellationToken);
        ...
        var resultAfter = await store.ListTasksAsync(cancellationToken: TestExecutionContext.Current.CancellationToken);
    ...
    public async Task MultipleOperations_ConcurrentAccess()
    {
        ...
        for (int i = 0; i < 10; i++)
        {
            ...
            {
                var metadata = new McpTaskMetadata();
                return await store.CreateTaskAsync(metadata, new RequestId($"req-{taskNum}"), new JsonRpcRequest { Method = "test" }, null, TestExecutionContext.Current.CancellationToken);
            }));
        }
    ...
    public async Task GetTaskAsync_ReturnsDefensiveCopy()
    {
        ...
        using var store = new InMemoryMcpTaskStore();
        var metadata = new McpTaskMetadata();
        var createdTask = await store.CreateTaskAsync(metadata, new RequestId("req-1"), new JsonRpcRequest { Method = "test" }, null, TestExecutionContext.Current.CancellationToken);

        // Act - Get the task and modify the returned copy
        var retrievedTask = await store.GetTaskAsync(createdTask.TaskId, null, TestExecutionContext.Current.CancellationToken);
        ...
        var taskAgain = await store.GetTaskAsync(createdTask.TaskId, null, TestExecutionContext.Current.CancellationToken);
    ...
    public async Task ListTasksAsync_ReturnsDefensiveCopies()
    {
        ...
        var directTask = await store.GetTaskAsync(originalTaskId, null, TestExecutionContext.Current.CancellationToken);
    ...
    public async Task CancelTaskAsync_ReturnsDefensiveCopy()
    {
        ...
        var cancelledTask = await store.CancelTaskAsync(createdTask.TaskId, null, TestExecutionContext.Current.CancellationToken);
    ...
    public async Task ConcurrentUpdates_HandlesContentionCorrectly()
    {
        // Arrange
        using var store = new InMemoryMcpTaskStore();
        var task = await store.CreateTaskAsync(new McpTaskMetadata(), new RequestId("req-1"), new JsonRpcRequest { Method = "test" }, null, TestExecutionContext.Current.CancellationToken);
        ...
        var updateTasks = Enumerable.Range(0, 100).Select(i =>
            Task.Run(async () =>
            {
                try
                {
                    await store.UpdateTaskStatusAsync(task.TaskId, McpTaskStatus.Working, $"Update {i}", null, TestExecutionContext.Current.CancellationToken);
                    return true;
                }
                catch
                {
        ...
        var finalTask = await store.GetTaskAsync(task.TaskId, null, TestExecutionContext.Current.CancellationToken);
    ...
    public async Task ListTasksAsync_PaginationWithCustomPageSize()
    {
        ...
        var result1 = await store.ListTasksAsync(cancellationToken: TestExecutionContext.Current.CancellationToken);
        var result2 = await store.ListTasksAsync(cursor: result1.NextCursor, cancellationToken: TestExecutionContext.Current.CancellationToken);
        var result3 = await store.ListTasksAsync(cursor: result2.NextCursor, cancellationToken: TestExecutionContext.Current.CancellationToken);
    ...
    public async Task ListTasksAsync_NoDuplicatesWithIdenticalTimestamps()
    {
        ...
        var createTasks = Enumerable.Range(0, 20).Select(i =>
            store.CreateTaskAsync(new McpTaskMetadata(), new RequestId($"req-{i}"), new JsonRpcRequest { Method = "test" }, null, TestExecutionContext.Current.CancellationToken));
        ...
        do
        {
            var result = await store.ListTasksAsync(cursor: cursor, cancellationToken: TestExecutionContext.Current.CancellationToken);
    ...
    public async Task UpdateTaskStatusAsync_ConcurrentWithList_NoCorruption()
    {
        ...
        for (int i = 0; i < 20; i++)
        {
            var task = await store.CreateTaskAsync(new McpTaskMetadata(), new RequestId($"req-{i}"), new JsonRpcRequest { Method = "test" }, null, TestExecutionContext.Current.CancellationToken);
            tasks.Add(task);
        }

        // Act - Concurrently list and update tasks
        var ct = TestExecutionContext.Current.CancellationToken;
        ...
        var updateTask = Task.Run(async () =>
        {
            ...
            {
                await store.UpdateTaskStatusAsync(task.TaskId, McpTaskStatus.Working, "Updated", null, TestExecutionContext.Current.CancellationToken);
    ...
    public async Task CreateTaskAsync_EnforcesMaxTasksLimit()
    {
        ...
        await store.CreateTaskAsync(metadata, new RequestId("req-1"), new JsonRpcRequest { Method = "test" }, null, TestExecutionContext.Current.CancellationToken);
        await store.CreateTaskAsync(metadata, new RequestId("req-2"), new JsonRpcRequest { Method = "test" }, null, TestExecutionContext.Current.CancellationToken);
        await store.CreateTaskAsync(metadata, new RequestId("req-3"), new JsonRpcRequest { Method = "test" }, null, TestExecutionContext.Current.CancellationToken);

        // Assert - Fourth task should throw
        var ex = await Assert.ThrowsAsync<InvalidOperationException>(() =>
            store.CreateTaskAsync(metadata, new RequestId("req-4"), new JsonRpcRequest { Method = "test" }, null, TestExecutionContext.Current.CancellationToken));
    ...
    public async Task CreateTaskAsync_EnforcesMaxTasksPerSessionLimit()
    {
        ...
        await store.CreateTaskAsync(metadata, new RequestId("req-1"), new JsonRpcRequest { Method = "test" }, "session-1", TestExecutionContext.Current.CancellationToken);
        await store.CreateTaskAsync(metadata, new RequestId("req-2"), new JsonRpcRequest { Method = "test" }, "session-1", TestExecutionContext.Current.CancellationToken);

        // Assert - Third task for session-1 should throw
        var ex = await Assert.ThrowsAsync<InvalidOperationException>(() =>
            store.CreateTaskAsync(metadata, new RequestId("req-3"), new JsonRpcRequest { Method = "test" }, "session-1", TestExecutionContext.Current.CancellationToken));
    ...
    public async Task CreateTaskAsync_MaxTasksPerSession_AllowsDifferentSessions()
    {
        ...
        var task3 = await store.CreateTaskAsync(metadata, new RequestId("req-3"), new JsonRpcRequest { Method = "test" }, "session-2", TestExecutionContext.Current.CancellationToken);
        var task4 = await store.CreateTaskAsync(metadata, new RequestId("req-4"), new JsonRpcRequest { Method = "test" }, "session-2", TestExecutionContext.Current.CancellationToken);
    ...
    public async Task CreateTaskAsync_MaxTasksPerSession_DoesNotApplyToNullSession()
    {
        ...
        var task1 = await store.CreateTaskAsync(metadata, new RequestId("req-1"), new JsonRpcRequest { Method = "test" }, null, TestExecutionContext.Current.CancellationToken);
        var task2 = await store.CreateTaskAsync(metadata, new RequestId("req-2"), new JsonRpcRequest { Method = "test" }, null, TestExecutionContext.Current.CancellationToken);
        var task3 = await store.CreateTaskAsync(metadata, new RequestId("req-3"), new JsonRpcRequest { Method = "test" }, null, TestExecutionContext.Current.CancellationToken);
    ...
    public async Task CreateTaskAsync_CombinesMaxTasksAndMaxTasksPerSession()
    {
        ...
        await store.CreateTaskAsync(metadata, new RequestId("req-4"), new JsonRpcRequest { Method = "test" }, "session-2", TestExecutionContext.Current.CancellationToken);
        await store.CreateTaskAsync(metadata, new RequestId("req-5"), new JsonRpcRequest { Method = "test" }, "session-2", TestExecutionContext.Current.CancellationToken);

        // Now global limit is reached (4 tasks total, but 5th would be 5)
        // Wait, we have 4 tasks, should be able to create one more
        await store.CreateTaskAsync(metadata, new RequestId("req-6"), new JsonRpcRequest { Method = "test" }, "session-3", TestExecutionContext.Current.CancellationToken);

        // Now at 5 tasks (global limit), should throw
        var ex = await Assert.ThrowsAsync<InvalidOperationException>(() =>
            store.CreateTaskAsync(metadata, new RequestId("req-7"), new JsonRpcRequest { Method = "test" }, "session-3", TestExecutionContext.Current.CancellationToken));
    ...
    public async Task CreateTaskAsync_MaxTasksPerSession_ExcludesExpiredTasks()
    {
        ...
        var task2 = await store.CreateTaskAsync(metadata, new RequestId("req-2"), new JsonRpcRequest { Method = "test" }, "session-1", TestExecutionContext.Current.CancellationToken);
