...
public class TaskCancellationIntegrationTests : ClientServerTestBase
{
    ...
    public async Task TaskTool_CancellationToken_FiresWhenTtlExpires()
    {
        ...
        await _toolStarted.Task.WaitAsync(TestConstants.DefaultTimeout, TestContext.CurrentContext.CancellationToken);

        // Assert - Wait for the cancellation to fire (should happen when TTL expires)
        var cancelled = await _toolCancellationFired.Task.WaitAsync(TestConstants.DefaultTimeout, TestContext.CurrentContext.CancellationToken);
    ...
    public async Task TaskTool_CancellationToken_FiresWhenExplicitlyCancelled()
    {
        ...
        var cancelledTask = await client.CancelTaskAsync(taskId, cancellationToken: TestContext.CurrentContext.CancellationToken);
    ...
    public async Task TaskTool_CompletesSuccessfully_WhenNotCancelled()
    {
        ...
        var task = await client.GetTaskAsync(callResult.Task.TaskId, cancellationToken: TestContext.CurrentContext.CancellationToken);
...
public class TaskCancellationConcurrencyTests : ClientServerTestBase
{
    ...
    public async Task CancelTask_OnlyCancelsTargetTask_NotOtherTasks()
    {
        ...
        await WaitForStart("task1", TestContext.CurrentContext.CancellationToken);
        await WaitForStart("task2", TestContext.CurrentContext.CancellationToken);

        // Act - Cancel only task1
        await client.CancelTaskAsync(result1.Task.TaskId, cancellationToken: TestContext.CurrentContext.CancellationToken);

        // Assert - task1 should be cancelled
        var task1Cancelled = await WaitForCancellation("task1", TestContext.CurrentContext.CancellationToken);
        ...
        var task2Status = await client.GetTaskAsync(result2.Task.TaskId, cancellationToken: TestContext.CurrentContext.CancellationToken);
        Assert.Equal(McpTaskStatus.Working, task2Status.Status);

        // Clean up - cancel task2
        await client.CancelTaskAsync(result2.Task.TaskId, cancellationToken: TestContext.CurrentContext.CancellationToken);
    }
    ...
    public async Task MultipleTasks_WithDifferentTtls_CancelIndependently()
    {
        ...
        await WaitForStart("short-ttl", TestContext.CurrentContext.CancellationToken);
        await WaitForStart("long-ttl", TestContext.CurrentContext.CancellationToken);

        // Assert - short TTL task should be cancelled automatically
        var shortCancelled = await WaitForCancellation("short-ttl", TestContext.CurrentContext.CancellationToken);
        ...
        var longTtlStatus = await client.GetTaskAsync(longTtlResult.Task.TaskId, cancellationToken: TestContext.CurrentContext.CancellationToken);
        Assert.Equal(McpTaskStatus.Working, longTtlStatus.Status);

        // Clean up
        await client.CancelTaskAsync(longTtlResult.Task.TaskId, cancellationToken: TestContext.CurrentContext.CancellationToken);
    }
}
...
#pragma warning restore CS0162
    ...
    public async Task CompletedTask_CannotTransitionToOtherStatus()
    {
        ...
        var cancelResult = await client.CancelTaskAsync(taskId, cancellationToken: TestContext.CurrentContext.CancellationToken);

        // Assert - Status should still be completed (not cancelled)
        Assert.Equal(McpTaskStatus.Completed, cancelResult.Status);

        // Verify via get
        var verifyStatus = await client.GetTaskAsync(taskId, cancellationToken: TestContext.CurrentContext.CancellationToken);
