...
public class McpClientResourceSubscriptionTests : ClientServerTestBase
{
    ...
    public async Task SubscribeToResourceAsync_WithHandler_DisposalUnsubscribes()
    {
        ...
        await Task.Delay(100, TestContext.CurrentContext.CancellationToken); // Allow time for notification to be processed
        ...
        await Task.Delay(100, TestContext.CurrentContext.CancellationToken); // Allow time to ensure notification is not processed
    ...
    public async Task SubscribeToResourceAsync_MultipleSubscriptions_BothReceiveNotifications()
    {
        ...
        // Assert
        using var cts = new CancellationTokenSource(TestConstants.DefaultTimeout);
        var combined = CancellationTokenSource.CreateLinkedTokenSource(cts.Token, TestContext.CurrentContext.CancellationToken);
        await Task.WhenAll(
    ...
    public async Task SubscribeToResourceAsync_MultipleHandlersSameUri_BothReceiveNotifications()
    {
        ...
        var combined2 = CancellationTokenSource.CreateLinkedTokenSource(cts2.Token, TestContext.CurrentContext.CancellationToken);
