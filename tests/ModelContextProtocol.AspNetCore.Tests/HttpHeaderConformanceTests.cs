...
public class HttpHeaderConformanceTests() : KestrelInMemoryTest(), IAsyncDisposable
{
    ...
    [TestCase("42", 42)]       // "42" header vs 42 body â†’ exact integer match
    [TestCase("42.0", 42)]     // "42.0" header vs 42 body â†’ numeric equivalence
    [TestCase("42", 42.0)]     // "42" header vs 42.0 body â†’ numeric equivalence
    ...
    public async Task Server_RejectsNonNumericMismatch_ForIntegerParam()
    {
        ...
        // Header says "99" but body says priority:42 â€” must reject even with numeric comparison
    ...
    public async Task Server_SkipsHeaderValidation_ForNonDraftVersion()
    {
        ...
        using var request = new HttpRequestMessage(HttpMethod.Post, "");
        request.Content = new StringContent(callJson, Encoding.UTF8, "application/json");
        // Send the WRONG header value â€” this should still succeed because version is non-draft
    ...
    public async Task Server_RejectsInvalidUtf8EncodedHeaderValue()
    {
        ...
        // Kestrel reads header bytes as Latin-1, so the UTF-8 bytes for "cafÃ©â˜•"
        // will be garbled and won't match the body value, causing rejection.
        var callJson = CallTool("cafÃ©â˜•", """{"region":"us-west1","priority":42,"verbose":false,"emptyVal":""}""");

        using var request = new HttpRequestMessage(HttpMethod.Post, "");
        request.Content = new StringContent(callJson, Encoding.UTF8, "application/json");
        request.Headers.Add("MCP-Protocol-Version", "DRAFT-2026-v1");
        request.Headers.TryAddWithoutValidation("Mcp-Method", "tools/call");
        // Raw UTF-8 non-ASCII value in Mcp-Name â€” server must reject this
        request.Headers.TryAddWithoutValidation("Mcp-Name", "cafÃ©â˜•");
        ...
        using var response = await utf8Client.SendAsync(request, TestExecutionContext.Current.CancellationToken);
        Assert.Equal(HttpStatusCode.BadRequest, response.StatusCode);
    }
