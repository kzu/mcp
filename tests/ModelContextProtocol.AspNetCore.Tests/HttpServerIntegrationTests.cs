...
public abstract class HttpServerIntegrationTests : LoggedTest
{
    private static SseServerIntegrationTestFixture? s_fixture;

    protected SseServerIntegrationTestFixture _fixture => s_fixture!;

    [OneTimeSetUp]
    public static async Task OneTimeSetUpAsync()
    {
        s_fixture = new SseServerIntegrationTestFixture();
    ...
    public void SetUp()
    {
        s_fixture!.Initialize(TestOutputHelper, ClientTransportOptions);
    }
    ...
    public async Task CallTool_EchoSessionId_ReturnsTheSameSessionId()
    {
        ...
        // act
        await using var client = await GetClientAsync();
        var result1 = await client.CallToolAsync("echoSessionId", cancellationToken: TestExecutionContext.Current.CancellationToken);
        var result2 = await client.CallToolAsync("echoSessionId", cancellationToken: TestExecutionContext.Current.CancellationToken);
        var result3 = await client.CallToolAsync("echoSessionId", cancellationToken: TestExecutionContext.Current.CancellationToken);
    ...
    public async Task ReadResource_Sse_TextResource()
    {
        ...
        var result = await client.ReadResourceAsync("test://static/resource/1", null, TestExecutionContext.Current.CancellationToken);
    ...
    public async Task GetPrompt_Sse_SimplePrompt()
    {
        ...
        // act
        await using var client = await GetClientAsync();
        var result = await client.GetPromptAsync("simple_prompt", null, cancellationToken: TestExecutionContext.Current.CancellationToken);
    ...
    public async Task GetPrompt_Sse_ComplexPrompt()
    {
        ...
        var result = await client.GetPromptAsync("complex_prompt", arguments, cancellationToken: TestExecutionContext.Current.CancellationToken);
    ...
    public async Task GetPrompt_Sse_NonExistent_ThrowsException()
    {
        ...
        // act
        await using var client = await GetClientAsync();
        await Assert.ThrowsAsync<McpProtocolException>(async () => await client.GetPromptAsync("non_existent_prompt", null, cancellationToken: TestExecutionContext.Current.CancellationToken));
    }
