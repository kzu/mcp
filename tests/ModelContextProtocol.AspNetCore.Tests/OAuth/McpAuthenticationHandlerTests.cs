...
public class McpAuthenticationHandlerTests() : KestrelInMemoryTest()
{
    ...
    public async Task Challenge_WithRelativeResourceMetadataUri_SetsAbsoluteUrl()
    {
        ...
        using var challengeResponse = await HttpClient.GetAsync(new Uri("/challenge", UriKind.Relative), HttpCompletionOption.ResponseHeadersRead, TestExecutionContext.Current.CancellationToken);
        ...
        using var metadataResponse = await HttpClient.GetAsync(new Uri(metadataPath, UriKind.Relative), TestExecutionContext.Current.CancellationToken);
    ...
    public async Task MetadataRequest_CustomResourceMetadataUriWithoutResource_ThrowsInvalidOperationException()
    {
        ...
        using var response = await HttpClient.GetAsync(new Uri(metadataPath, UriKind.Relative), TestExecutionContext.Current.CancellationToken);
    ...
    public async Task Challenge_WithAbsoluteResourceMetadataUri_SetsConfiguredUrl()
    {
        ...
        using var metadataResponse = await HttpClient.GetAsync(metadataUri, TestExecutionContext.Current.CancellationToken);
        metadataResponse.EnsureSuccessStatusCode();
    }
    ...
    public async Task MetadataRequest_WithHostMismatch_LogsWarning()
    {
        ...
        using var metadataResponse = await HttpClient.SendAsync(metadataRequest, TestExecutionContext.Current.CancellationToken);
    ...
    public async Task Challenge_WithDefaultMetadata_ComposesResourceSpecificEndpoint()
    {
        ...
        using var challengeResponse = await HttpClient.GetAsync(new Uri("/resource/tools/list", UriKind.Relative), HttpCompletionOption.ResponseHeadersRead, TestExecutionContext.Current.CancellationToken);
    ...
    public async Task Challenge_WithDefaultMetadata_AndPathBase_ComposesResourceSpecificEndpoint()
    {
        ...
        using var challengeResponse = await HttpClient.GetAsync(new Uri("/api/resource/tools/list", UriKind.Relative), HttpCompletionOption.ResponseHeadersRead, TestExecutionContext.Current.CancellationToken);
    ...
    public async Task MetadataRequest_DefaultEndpoint_SetsResourceFromSuffix()
    {
        ...
        using var metadataResponse = await HttpClient.GetAsync(new Uri("/.well-known/oauth-protected-resource/resource/tools", UriKind.Relative), TestExecutionContext.Current.CancellationToken);
    ...
    public async Task MetadataRequest_DefaultEndpoint_WithPathBase_SetsResourceFromSuffix()
    {
        ...
        using var metadataResponse = await HttpClient.GetAsync(new Uri("/api/.well-known/oauth-protected-resource/resource/tools", UriKind.Relative), TestExecutionContext.Current.CancellationToken);
