...
public class TokenCacheTests : OAuthTestBase
{
    ...
    public async Task GetTokenAsync_CachedAccessTokenIsUsedForOutgoingRequests()
    {
        ...
        await using (var setupClient = await McpClient.CreateAsync(setupTransport, loggerFactory: LoggerFactory, cancellationToken: TestExecutionContext.Current.CancellationToken))
        {
