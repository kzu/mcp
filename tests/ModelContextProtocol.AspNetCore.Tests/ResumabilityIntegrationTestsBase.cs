...
public abstract class ResumabilityIntegrationTestsBase() : KestrelInMemoryTest()
{
    ...
    public virtual async Task Client_CanPollResponse_FromServer()
    {
        ...
        var progressTool = McpServerTool.Create(async (RequestContext<CallToolRequestParams> context, IProgress<ProgressNotificationValue> progress) =>
        {
            ...
            await clientReceivedInitialValueTcs.Task.WaitAsync(TestExecutionContext.Current.CancellationToken);
            ...
            await clientReceivedPolledValueTcs.Task.WaitAsync(TestExecutionContext.Current.CancellationToken);
        ...
        var result = await client.CallToolAsync(ProgressToolName, progress: progressHandler, cancellationToken: TestExecutionContext.Current.CancellationToken);
    ...
    public virtual async Task Client_CanResumePostResponseStream_AfterDisconnection()
    {
        ...
        var progressTool = McpServerTool.Create(async (RequestContext<CallToolRequestParams> context, IProgress<ProgressNotificationValue> progress, CancellationToken cancellationToken) =>
        {
            ...
            var reconnectAttempt = await faultingStreamHandler.TriggerFaultAsync(TestExecutionContext.Current.CancellationToken);
            ...
            await clientReceivedReconnectValueTcs.Task.WaitAsync(TestExecutionContext.Current.CancellationToken);
...
#pragma warning restore MCPEXP002
        ...
        var server = await serverTcs.Task.WaitAsync(timeout, TestExecutionContext.Current.CancellationToken);
        ...
        await faultingStreamHandler.WaitForUnsolicitedMessageStreamAsync(TestExecutionContext.Current.CancellationToken);

        // Send a custom notification to the client on the unsolicited message stream
        await server.SendNotificationAsync(CustomNotificationMethod, new JsonObject { ["message"] = InitialMessage }, cancellationToken: TestExecutionContext.Current.CancellationToken);

        // Wait for client to receive the first notification
        await clientReceivedInitialNotificationTcs.Task.WaitAsync(timeout, TestExecutionContext.Current.CancellationToken);
        ...
        await server.SendNotificationAsync(CustomNotificationMethod, new JsonObject { ["message"] = ReplayedMessage }, cancellationToken: TestExecutionContext.Current.CancellationToken);

        // Allow the client to reconnect
        reconnectAttempt.Continue();

        // Wait for client to receive the notification via replay
        await clientReceivedReplayedNotificationTcs.Task.WaitAsync(timeout, TestExecutionContext.Current.CancellationToken);

        // Send a final notification while the client has reconnected - this should be handled by the transport
        await server.SendNotificationAsync(CustomNotificationMethod, new JsonObject { ["message"] = ReconnectMessage }, cancellationToken: TestExecutionContext.Current.CancellationToken);

        // Wait for the client to receive the final notification
        await clientReceivedReconnectNotificationTcs.Task.WaitAsync(timeout, TestExecutionContext.Current.CancellationToken);
    ...
    public virtual async Task Server_Returns400_WhenLastEventIdRefersToWrongSession()
    {
        ...
        var initResponse = await HttpClient.SendAsync(initRequest, HttpCompletionOption.ResponseHeadersRead, TestExecutionContext.Current.CancellationToken);
        ...
        await using var initStream = await initResponse.Content.ReadAsStreamAsync(TestExecutionContext.Current.CancellationToken);
        string? eventId = null;
        await foreach (var sseItem in SseParser.Create(initStream).EnumerateAsync(TestExecutionContext.Current.CancellationToken))
        {
        ...
        var resumeResponse = await HttpClient.SendAsync(resumeRequest, HttpCompletionOption.ResponseHeadersRead, TestExecutionContext.Current.CancellationToken);
        ...
        var initResponse2 = await HttpClient.SendAsync(initRequest2, HttpCompletionOption.ResponseHeadersRead, TestExecutionContext.Current.CancellationToken);
        ...
        await using var initStream2 = await initResponse2.Content.ReadAsStreamAsync(TestExecutionContext.Current.CancellationToken);
        await foreach (var _ in SseParser.Create(initStream2).EnumerateAsync(TestExecutionContext.Current.CancellationToken))
        {
            // Consume the stream
        }
        ...
        var mismatchResponse = await HttpClient.SendAsync(mismatchRequest, HttpCompletionOption.ResponseHeadersRead, TestExecutionContext.Current.CancellationToken);
        ...
        var responseBody = await mismatchResponse.Content.ReadAsStringAsync(TestExecutionContext.Current.CancellationToken);
    ...
    public virtual async Task EnablePollingAsync_SendsSseItemWithRetryField()
    {
        ...
        var result = await client.CallToolAsync(PollingToolName, cancellationToken: TestExecutionContext.Current.CancellationToken);
    ...
    protected async Task<SseResponse> SendInitializeAndReadSseResponseAsync(string initializeRequest)
    {
        ...
        await using var stream = await response.Content.ReadAsStreamAsync(TestExecutionContext.Current.CancellationToken);
        await foreach (var sseItem in SseParser.Create(stream).EnumerateAsync(TestExecutionContext.Current.CancellationToken))
        {
            if (!string.IsNullOrEmpty(sseItem.EventId))
            {
