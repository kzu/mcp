...
public class MapMcpSseTests() : MapMcpTests()
{
    ...
    public async Task Allows_Customizing_Route(string pattern)
    {
        ...
        using var response = await HttpClient.GetAsync($"http://localhost:5000{pattern}/sse", HttpCompletionOption.ResponseHeadersRead, TestExecutionContext.Current.CancellationToken);
        response.EnsureSuccessStatusCode();
        using var sseStream = await response.Content.ReadAsStreamAsync(TestExecutionContext.Current.CancellationToken);
        using var sseStreamReader = new StreamReader(sseStream, System.Text.Encoding.UTF8);
        var eventLine = await sseStreamReader.ReadLineAsync(TestExecutionContext.Current.CancellationToken);
        var dataLine = await sseStreamReader.ReadLineAsync(TestExecutionContext.Current.CancellationToken);
