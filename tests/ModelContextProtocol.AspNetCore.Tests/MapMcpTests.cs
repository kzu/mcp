...
public abstract class MapMcpTests() : KestrelInMemoryTest()
{
        ...
        McpClientOptions? clientOptions = null)
    {
        ...
        return await McpClient.CreateAsync(transport, clientOptions, LoggerFactory, TestExecutionContext.Current.CancellationToken);
    }
    ...
    public async Task LongRunningToolCall_DoesNotTimeout_WhenNoEventStreamStore()
    {
        ...
        for (var attempt = 0; attempt < 3; attempt++)
        {
            try
            {
                ...
                await using var mcpClient = await McpClient.CreateAsync(transport, loggerFactory: LoggerFactory, cancellationToken: TestExecutionContext.Current.CancellationToken);
    ...
    public async Task OutgoingFilter_MultipleFilters_ExecuteInOrder()
    {
        ...
        using var allFiltersCts = CancellationTokenSource.CreateLinkedTokenSource(TestExecutionContext.Current.CancellationToken);
    ...
    public async Task OutgoingFilter_CanSendAdditionalMessages()
    {
        ...
        var extraMessage = await extraReceived.Task.WaitAsync(TimeSpan.FromSeconds(10), TestExecutionContext.Current.CancellationToken);
