...
public abstract class MapMcpTests() : KestrelInMemoryTest()
{
    ...
    {
        if (Stateless) Assert.Ignore("User validation across requests is not applicable in stateless mode.");
    ...
    {
        if (Stateless) Assert.Ignore("Sampling is not supported in stateless mode.");
    ...
    {
        if (Stateless) Assert.Ignore("Server-originated requests are not supported in stateless mode.");
