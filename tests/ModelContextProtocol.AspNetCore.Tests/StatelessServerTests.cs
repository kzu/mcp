...
public class StatelessServerTests : KestrelInMemoryTest, IAsyncDisposable
{
    ...
    private Task<McpClient> ConnectMcpClientAsync(McpClientOptions? clientOptions = null)
        => McpClient.CreateAsync(
            ...
            clientOptions, LoggerFactory, TestContext.CurrentContext.CancellationToken);
    ...
    public async Task EnablingStatelessMode_Disables_GetAndDeleteEndpoints()
    {
        ...
        using var getResponse = await HttpClient.GetAsync("/", TestContext.CurrentContext.CancellationToken);
        Assert.Equal(HttpStatusCode.MethodNotAllowed, getResponse.StatusCode);

        using var deleteResponse = await HttpClient.DeleteAsync("/", TestContext.CurrentContext.CancellationToken);
    ...
    public async Task SamplingRequest_Fails_WithInvalidOperationException()
    {
        ...
        var toolResponse = await client.CallToolAsync("testSamplingErrors", cancellationToken: TestContext.CurrentContext.CancellationToken);
    ...
    public async Task RootsRequest_Fails_WithInvalidOperationException()
    {
        ...
        var toolResponse = await client.CallToolAsync("testRootsErrors", cancellationToken: TestContext.CurrentContext.CancellationToken);
        var toolContent = Assert.Single(toolResponse.Content);
        Assert.Equal("Server to client requests are not supported in stateless mode.", Assert.IsType<TextContentBlock>(toolContent).Text);
    }
    ...
    public async Task ElicitRequest_Fails_WithInvalidOperationException()
    {
        ...
        var toolResponse = await client.CallToolAsync("testElicitationErrors", cancellationToken: TestContext.CurrentContext.CancellationToken);
        var toolContent = Assert.Single(toolResponse.Content);
        Assert.Equal("Server to client requests are not supported in stateless mode.", Assert.IsType<TextContentBlock>(toolContent).Text);
    }
...
#pragma warning disable MCPEXP002 // RunSessionHandler is experimental
                options.RunSessionHandler = async (context, server, cancellationToken) =>
                {
                    unsolicitedNotificationException = await Assert.ThrowsAsync<InvalidOperationException>(
                        () => server.SendNotificationAsync(NotificationMethods.PromptListChangedNotification, TestContext.CurrentContext.CancellationToken));
...
#pragma warning restore MCPEXP002
    ...
    public async Task ScopedServices_Resolve_FromRequestScope()
    {
        ...
        var toolResponse = await client.CallToolAsync("testScope", cancellationToken: TestContext.CurrentContext.CancellationToken);
        var toolContent = Assert.Single(toolResponse.Content);
    ...
    public async Task ProgressNotifications_Work_InStatelessMode()
    {
        ...
        await progressReceived.Task.WaitAsync(TimeSpan.FromSeconds(30), TestContext.CurrentContext.CancellationToken);
    ...
    public async Task ConfigureSessionOptions_RunsPerRequest_InStatelessMode()
    {
        ...
        var toolResponse1 = await client1.CallToolAsync("dynamicTool", cancellationToken: TestContext.CurrentContext.CancellationToken);
        ...
        var toolResponse2 = await client2.CallToolAsync("dynamicTool", cancellationToken: TestContext.CurrentContext.CancellationToken);
