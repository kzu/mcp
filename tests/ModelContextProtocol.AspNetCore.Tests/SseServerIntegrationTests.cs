...
public class SseServerIntegrationTests : HttpServerIntegrationTests
{
    ...
    {
        using var sseResponse = await _fixture.HttpClient.GetAsync("/sse", HttpCompletionOption.ResponseHeadersRead, TestExecutionContext.Current.CancellationToken);
    ...
    public async Task EventSourceStream_Includes_MessageEventType()
    {
        ...
        await using var sseResponse = await _fixture.HttpClient.GetStreamAsync("/sse", TestExecutionContext.Current.CancellationToken);
        using var streamReader = new StreamReader(sseResponse);

        var endpointEvent = await streamReader.ReadLineAsync(TestExecutionContext.Current.CancellationToken);
        Assert.Equal("event: endpoint", endpointEvent);

        var endpointData = await streamReader.ReadLineAsync(TestExecutionContext.Current.CancellationToken);
        ...
        {
            using var response = await _fixture.HttpClient.PostAsync(messageEndpoint, initializeRequestBody, TestExecutionContext.Current.CancellationToken);
        ...
        {
            using var response = await _fixture.HttpClient.PostAsync(messageEndpoint, initializedNotificationBody, TestExecutionContext.Current.CancellationToken);
            Assert.Equal(HttpStatusCode.Accepted, response.StatusCode);
        }

        Assert.Equal("", await streamReader.ReadLineAsync(TestExecutionContext.Current.CancellationToken));
        var messageEvent = await streamReader.ReadLineAsync(TestExecutionContext.Current.CancellationToken);
