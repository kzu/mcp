...
public partial class SseIntegrationTests() : KestrelInMemoryTest()
{
    ...
    public async Task ConnectAndReceiveMessage_InMemoryServer()
    {
        ...
        await mcpClient.SendNotificationAsync("test/message", new Envelope { Message = "Hello, SSE!" }, serializerOptions: JsonContext.Default.Options, cancellationToken: TestContext.CurrentContext.CancellationToken);
...
#pragma warning restore MCPEXP002
        ...
        // Send a test message through POST endpoint
        await mcpClient.SendNotificationAsync("test/notification", new Envelope { Message = "Hello from client!" }, serializerOptions: JsonContext.Default.Options, cancellationToken: TestContext.CurrentContext.CancellationToken);

        var message = await receivedNotification.Task.WaitAsync(TestConstants.DefaultTimeout, TestContext.CurrentContext.CancellationToken);
    ...
    public async Task Completion_ServerShutdown_ReturnsHttpCompletionDetails()
    {
        ...
        var details = await mcpClient.Completion.WaitAsync(TestContext.CurrentContext.CancellationToken);
